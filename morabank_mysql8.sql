...
CREATE TABLE IF NOT EXISTS accounts (
  ...
  COLLATE=utf8mb4_unicode_ci
  COMMENT='Tabla de cuentas bancarias (sin referencia a banks)';
...
CREATE TABLE IF NOT EXISTS users (
  ...
  CONSTRAINT fk_user_account FOREIGN KEY (account_iban)
  ...
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='Usuarios del sistema (cedula, con email, tipo y FK a cuentas)';
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  CONSTRAINT fk_trans_origin FOREIGN KEY (origin_iban)
    REFERENCES accounts (iban)
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
  CONSTRAINT fk_trans_destination FOREIGN KEY (destination_iban)
    REFERENCES accounts (iban)
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
  CHECK (amount > 0)
  ...
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='Registro de transacciones (origen y destino en IBAN)';
...
CREATE TABLE IF NOT EXISTS mfa_codes (
  ...
  CONSTRAINT fk_mfa_user FOREIGN KEY (user_id)
    REFERENCES users (identification)
    ON UPDATE CASCADE
    ON DELETE CASCADE
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='Códigos MFA enviados por correo.';
